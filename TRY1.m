...
%For a given interest rate, wage and productivity 
A=1;
...
y=fsolve(@(guess) SolvEco(guess,L,A,PARAM),GUESS);
y=abs(y)

%Let's do some verif
verif=SolvEco(y,L,A,PARAM)
Economy(y(1),N,y(2),A,PARAM)

%AMAZING


%Let's do a few plots
Kss=y(1)    % Do you just claim that this is the correct ss 
Rss=y(2)

ECO=Economy(Kss,N,Rss,A,PARAM)

AO=AgentOptim(ECO(2),ECO(3),A,L,PARAM)
figure(2)
plot(1:20,AO(:,1),"b--",1:20,AO(:,2),"r-")

Yss=F(Kss,N,Rss,A,PARAM)
...
%% 2/ Ressources are extracted at an increasing cost
